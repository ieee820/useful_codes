...
des_dir = '/data/share_data/crnn_train/gen_no_lstm/100w/'
...
image_label = readfile('./balance_sample_len10_25w.txt')
print(image_label)
count = 0
for line in image_label:
    ...
    for root, d, fs in list_dir:
        for file in fs:
            ...
            draw.text((0 + radX, 0 + radY), line, fill=(color_value, color_value, color_value), font=ttfont)
            ...
            region = im.crop(box)
            if (radX + rX) >= width:
                print("over flow width, skip: ", image_path)
                continue
            index = index + 1
            ucode = ord(line[0])
            output_dir = os.path.join(des_dir, str(ucode))
            if not os.path.isdir(output_dir):
                os.makedirs(output_dir)
            filename = output_dir + '/' + str(ucode) + '_' + str(index) + '.jpg'
            ...
            #blur
            if select >= 12 and select < 14:
                numpy_region = np.asarray(region)
                sigma = random.uniform(1.0, 2.5)
                blurer = iaa.GaussianBlur(sigma)
                numpy_region = blurer.augment_image(numpy_region)
                region = Image.fromarray(numpy_region)
                # print('blur: ', filename)
            #elastic
            if select >= 14 and select < 16:
                numpy_region = np.asarray(region)
                sigma = random.uniform(0.6, 1.0)
                blurer = iaa.ElasticTransformation(alpha=2.5, sigma=sigma)
                numpy_region = blurer.augment_image(numpy_region)
                region = Image.fromarray(numpy_region)
            #crop and pad
            if select >= 16 and select < 18:
                numpy_region = np.asarray(region)
                blurer = iaa.Crop(percent=(0.01, 0.05))
                numpy_region = blurer.augment_image(numpy_region)
                region = Image.fromarray(numpy_region)
